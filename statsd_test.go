...
func TestUDPNotListening(t *testing.T) {
	...
	c, err := New()
	if c.muted {
		t.Error("client should not mute")
	}
	if err != nil {
		t.Error("client with non-listening port should not error")
	}
}
...
func BenchmarkWithReconnect(b *testing.B) {
	serv := newServer(b, "udp", testAddr, func([]byte) {})
	c, err := New(
		Address(serv.addr),
		FlushPeriod(0),
		LazyConnect(),
		FlushesBetweenReconnect(10),
